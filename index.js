...
async function handleRequest(request) {
  ...
  if (url.pathname === "/") {
    return Response.redirect("https://github.com/ihciah", 302);
  }
  if (!url.pathname.startsWith("/v2/")) {
    return new Response("invalid route.", {status: 400});
  }
  if (url.pathname.startsWith("/v2/movie/search")) {
    return movieSearch(url);
  }
  return proxyRequest(url);
}

async function movieSearch(url) {
  const webURL = new URL("https://search.douban.com/movie/subject_search");
  webURL.searchParams.set("search_text", url.searchParams.get("q"));
  ...
  const regexp = new RegExp(`window\.__DATA__ = "(.*?)"`);
  const matches = regexp.exec(body);
  if (matches.length < 2) {
    return new Response("unable to parse html", {status: 200, headers: new Headers({
        "content-type": "application/json; charset=utf-8"
      })});
  }
  var j = decrypt(matches[1]);
  console.log(j);
  return new Response(json2api(j), {status: 200, headers: new Headers({
      "content-type": "application/json; charset=utf-8"
    })});
}

async function proxyRequest(url) {
  url.host = "api.douban.com";
...
function json2api(j) {
  const api = {start: 0, subjects: [], title: "Search Results"};
  for (let i = 0; i < j.payload.items.length; i++) {
    let item = j.payload.items[i];
    if (item["tpl_name"] === "search_subject") {
      api.subjects.push({
        "rating": {"max": 10, "min": 0, "average": item.rating.value, "stars": item.rating["star_count"] * 10},
        "title": item.title,
        "original_title": item.title,
        "year": "unknown",
        "collect_count": item.rating.count,
        "id": item.id,
        "alt": item.url,
        "subtype": "movie",
      });
